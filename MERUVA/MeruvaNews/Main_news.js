...
(function () {
  ...
  /* Màu nhấn theo nhánh — PHẢI khớp :root trong Main_news.css.
     Lần đổi bảng màu trước chỉ sửa file CSS, bảng này bị bỏ sót nên
     JS ghi đè lại bằng vàng/teal cũ. Đã đồng bộ:
       coffee #D4A437 → #D2402A   ·   stay #3D9E8C → #2D7D6E
       news/promo: vàng cũ → ấn son (hai khoá này chỉ có ở đây). */
  var AC = {
    service: '#3B7BC4', '3d': '#7EB8C8', bakery: '#C8A040',
    coffee: '#D2402A', stay: '#2D7D6E', card: '#B0B0C0',
    news: '#D2402A', promo: '#D2402A'
  };
  ...
  POSTS.forEach(function (a) {
    ...
    html +=
      '<article class="entry' + (a.image ? ' has-thumb' : '') + '" style="--ac:' + (AC[a.accent] || AC.news) + '" ' +
        ...
        '</div>' +
        (a.image ? '<img class="thumb" src="' + esc(a.image) + '" alt="' + esc(a.imageAlt) + '" loading="lazy">' : '') +
        '<div class="go">→</div>' +
      '</article>';
  });
  if (lastDate !== null) html += '</div>';
